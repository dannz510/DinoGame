...
void initPoissonSamples( const in vec2 randomSeed ) {
  ...
  // jsfiddle that shows sample pattern: https://jsfiddle.ly/a16ff1p7/
...
class BasicWorldDemo {
  constructor() {
    ...
    // Add touch event listener for starting the game as well
    document.getElementById('game-menu').addEventListener('touchstart', (msg) => this._OnStart(msg), false);
  }

  _OnStart(msg) {
    document.getElementById('game-menu').style.display = 'none';
    this._gameStarted = true;
    // Hide game over screen if it's visible when starting a new game
  ...
  _ResetGame() {
    ...
    if (this.world_) {
      this.world_.Cleanup(); // We will add a cleanup method to WorldManager
    }
    ...
    if (this.background_) {
        this.background_.Cleanup(); // Add a cleanup method to Background
    }
    ...
    document.getElementById('game-over').classList.remove('active'); // Hide game over screen
    document.getElementById('score-text').innerText = '00000'; // Reset score display
  }
  ...
  Step_(timeElapsed) {
    ...
    if (this.player_.gameOver && !this.gameOver_) {
      this.gameOver_ = true;
      document.getElementById('game-over').classList.add('active'); // Use add for clarity
      // Add event listener to restart the game on tap
      document.getElementById('game-over').addEventListener('touchstart', () => this._ResetGame(), {once: true});
      document.getElementById('game-over').addEventListener('click', () => this._ResetGame(), {once: true});
    }
  }
}
